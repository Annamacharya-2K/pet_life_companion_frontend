...
import { SIGNIN_URL, USER_SIGNIN_URL } from '../URLconstants';

const UserAuth = ({type,onLogin}) => {
    ...
    const onSignIn = async () => {
        ...
        try {
            const response = await axios({
                method: 'post', url: (type.name==="admin"?SIGNIN_URL:USER_SIGNIN_URL), data: {
                    username: username,
                    password: password,
                }
            });
            ...
            if (response && response.status === 200) {
                ...
                type.name==="admin"?history("/adminpage"):history("/")
                ...
                <div className='row'>
                    ...
                    <div className='col-sm-6'>
                        <div className={`vertical-center border-style login-page`}>
                            ...
                            <button className='btn btn-custom btn-lg' type={'submit'}  onClick={() => { history('/usersignup') }} disabled={btndisabled} style={{marginLeft:'10px'}}>Sign Up</button>
                            </div>
